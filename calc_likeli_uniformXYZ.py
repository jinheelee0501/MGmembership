...
def calc_likeli_uniformXYZ(group_name,X,Y,Z,dX,dY,dZ,a1,a2,a3,U,V,W,dU,dV,dW,b1,b2,b3,ra,de,pra,epra,pde,epde,v=False,ev=False,d=False,ed=False,lencon=1.0,sharpsmooth='-'):
...
#    Rot_DFin = np.dot(R1_D,Rot_D1)
    ...
    infile = pdir+'PriorPDF_rv_dist_b_pm_%s_%.1f_uniformXYZ.txt' %(group_name,10000000.0)
...
#    K = 4.74057
    ...
    for iid, ii in enumerate(distrange): # 50 iteration #(len(dist)):
        ...
        dXX = dX*lencon ; dYY = dY*lencon ; dZZ = dZ*lencon
        volume = 4*np.pi/3.*dXX*dYY*dZZ

        if sharpsmooth == 'sh':
            p = 1./volume
            ellip = (xp-X)**2/dXX**2 + (yp-Y)**2/dYY**2 + (zp-Z)**2/dZZ**2
            if ellip < 1.: likeX = p**(1/3.) ;likeY = likeX ; likeZ = likeX
            else: likeX = 0 ;likeY = 0; likeZ = 0.
        elif sharpsmooth == 'sm':
            B=0.2
            pin = 0.9/volume
            pout = pin*np.exp(0.5/B**2 - 0.5*((xp-X)/dX/B)**2 - 0.5*((yp-Y)/dY/B)**2 - 0.5*((zp-Z)/dZ/B)**2)
            ellip = (xp-X)**2/dXX**2 + (yp-Y)**2/dYY**2 + (zp-Z)**2/dZZ**2
            if ellip < 1.: likeX = pin**(1/3.) ; likeY = likeX ; likeZ = likeX
            else: likeX = pout**(1./3.) ; likeY = likeX ; likeZ = likeX
...
#    return result
    ...
    return result[0],result[1],result[2],likeX,likeY,likeZ,likeU,likeV,likeW#,x,y,z,u,v,w,rv,dist,pmra_obs,pmde_obs
